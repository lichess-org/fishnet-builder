...
def init_ms_env_compile(env):
    ...
    if env['clang-cl']:
        ...
        flags += ' -Wno-nonportable-include-path -Wno-sizeof-pointer-memaccess ' 
...
def init_gnu_env_link_tool(env):
    ...
    if env.on_linux():
        # check if we need to use pin3dwarf or pindwarf
        olddwarf = env.expand('%(pin)s/intel64/lib-ext/libpin3dwarf.so')
        newdwarf = env.expand('%(pin)s/intel64/lib/libpindwarf.so')
        if os.path.exists(olddwarf):
            libs += ' -lpin3dwarf'
        elif os.path.exists(newdwarf):
            libs += ' -lpindwarf'
        else:
