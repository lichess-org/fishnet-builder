FROM --platform=linux/amd64 docker.io/alpine:3.23.4 AS fishnet-builder-amd64
WORKDIR /fishnet
...
RUN rustup-init -y --no-modify-path --profile minimal --default-toolchain 1.95.0 --target x86_64-unknown-linux-musl
...
FROM --platform=linux/arm64 docker.io/alpine:3.23.4 AS fishnet-builder-arm64
WORKDIR /fishnet
...
ENV CARGO_HOME=/usr/local/cargo \
    PATH=/usr/local/cargo/bin:$PATH
RUN rustup-init -y --no-modify-path --profile minimal --default-toolchain 1.95.0 --target aarch64-unknown-linux-musl
RUN cargo install cargo-auditable
ENV COMP=clang \
    LDFLAGS=-static
