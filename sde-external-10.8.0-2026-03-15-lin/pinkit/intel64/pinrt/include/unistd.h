...
pid_t _Fork(void);
#if defined(PIN_RT)
pid_t fork_with_clone(uintptr_t, uint32_t, uintptr_t*);
pid_t _Fork_With_Clone(uintptr_t, uint32_t, uintptr_t*);
#endif
...
PINCRT_DLL_DATA_VIS char *optarg;
PINCRT_DLL_DATA_VIS int optind;
PINCRT_DLL_DATA_VIS int opterr;
PINCRT_DLL_DATA_VIS int optopt;
#else
extern char *optarg;
extern int optind, opterr, optopt;
#endif

_arch_long pathconf(const char *, int);
_arch_long fpathconf(int, int);
_arch_long sysconf(int);
...
int acct(const char *);
_arch_long syscall(_arch_long, ...);
int execvpe(const char *, char *const [], char *const []);
int issetugid(void);
int getentropy(void *, size_t);
#if defined(TARGET_WINDOWS)
PINCRT_DLL_DATA_VIS int optreset;
#else
extern int optreset;
#endif
#endif
...
#ifdef _GNU_SOURCE
#if defined(TARGET_WINDOWS)
PINCRT_DLL_DATA_VIS char** environ;
#else
extern char **environ;
#endif
