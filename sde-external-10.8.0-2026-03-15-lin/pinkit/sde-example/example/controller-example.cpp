...
static CACHELINE_COUNTER global_addr_counter[SEND + 1];
static CACHELINE_COUNTER tid_addr_counter[SEND + 1];
static CACHELINE_COUNTER global_ins_counter;
...
{
    global_addr_counter[pct]._count++;
    if (tid == control_tid)
...
int main(int argc, char* argv[])
{
    ...
    {
    	global_addr_counter[i]._count = 0;
    	tid_addr_counter[i]._count = 0;
    }
    global_ins_counter._count = 0;
